...
export function MilestoneBadges({ goal, milestones, rewards = [] }: { goal: Goal; milestones: Milestone[]; rewards?: Reward[] }) {
        ...
        return (
          <li key={m.id} className={`tile flex items-center gap-3 px-3 py-2.5 ${done ? "soft-mint" : isNext ? "bg-white shadow-[var(--shadow-card)] ring-2 ring-blue/30" : "soft-cloud"}`}>
            ...
            <span className="min-w-0">
              ...
              <span className="block text-xs t-muted truncate max-w-[10rem]">{done ? `behaald ${fmtDate(m.achieved_at, "d MMM")}` : m.name}</span>
            </span>
