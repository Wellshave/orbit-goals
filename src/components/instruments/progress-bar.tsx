...
export function ProgressBar({ goal, milestones = [], height = "md", accent }: { goal: Goal; milestones?: Milestone[]; height?: "sm" | "md" | "lg"; accent?: string }) {
  ...
  return (
    <div className={`relative w-full ${h} rounded-full bg-cloud overflow-visible`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(p * 100)} aria-label={`Voortgang ${Math.round(p * 100)}%`}>
      ...
      {!done && e > 0 && e < 1 && (
        <span aria-hidden title="Verwacht volgens planning" className="absolute -top-1 -bottom-1 w-0.5 rounded bg-ink/25" style={{ left: `${e * 100}%` }} />
      )}
      {milestones.map((m) => (
