...
import { periodLabel, periodWord } from "@/lib/periods";
    ...
    <article className="card hover-lift p-5 flex flex-col gap-3" data-tour="kpi-card">
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm t-muted mt-1">{explainKpi(t, view)}</p>
        </div>
      ...
      <div className="flex items-center justify-between gap-2 text-xs t-muted">
        <span className="flex items-center gap-2"><AvatarStack people={people} size="xs" />{view.streak >= 2 && <span className="inline-flex items-center gap-0.5 text-coral-deep font-bold"><Flame className="size-3.5" aria-hidden /> {t("kpis.inARow", { n: view.streak })}</span>}</span>
        <span>{explainKpiChange(t, view, pw) ?? ""}</span>
      </div>
      {showCheckin && !view.openPeriod.done && (
        <Link href={`/kpis/${kpi.id}`} className="press inline-flex items-center justify-center rounded-full bg-mintsoft text-mint-deep font-semibold text-sm py-2 hover:bg-mint hover:text-ink">{t("kpis.fillCheckin", { p: periodLabel(kpi.frequency, view.openPeriod.start, locale) })}</Link>
      )}
    </article>
  );
}
