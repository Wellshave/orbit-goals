      ...
      <div className="flex flex-col gap-6">
        ...
        <Panel eyebrow="Rechten" title="Wat jij mag">
          <ul className="text-sm grid sm:grid-cols-2 gap-x-6 gap-y-1.5 text-ink-2">
            <li>Rol: <strong className="text-ink">{ROLE_LABELS[profile.role]}</strong></li>
            <li>Persoonlijke doelen en KPI&apos;s aanmaken: <strong className="text-ink">ja</strong></li>
            <li>Team- en company goals aanmaken: <strong className="text-ink">{isAdmin ? "ja" : "nee"}</strong></li>
            <li>KPI&apos;s toewijzen aan anderen: <strong className="text-ink">{isAdmin ? "ja" : "nee"}</strong></li>
            <li>Teamleden uitnodigen en beheren: <strong className="text-ink">{isAdmin ? "ja" : "nee"}</strong></li>
            <li>Privédoelen van anderen zien: <strong className="text-ink">nooit</strong></li>
          </ul>
          <div className="mt-4 text-xs t-muted">
            <p className="t-label mb-1">Zichtbaarheidsniveaus</p>
        ...
        {isAdmin && (
          <>
              ...
              <ul className="divide-y divide-line">
                {dir.members.map((m) => (
                  <li key={m.id} className="py-3 grid grid-cols-[auto_1fr_auto] items-center gap-3">
                    ...
                    <span className="min-w-0"><Link href={`/people/${m.id}`} className="block text-sm font-bold truncate hover:text-blue-deep">{m.full_name}{m.id === profile.id ? " (jij)" : ""}</Link><span className="block text-xs t-muted truncate">{m.email} · {m.job_title || "—"} · {dir.teamsOf(m.id).map((t) => t.name).join(", ") || "geen team"}</span></span>
            ...
            <Panel eyebrow="Uitnodigen" title="Teamleden uitnodigen" id="uitnodigen">
              <p className="text-sm t-muted mb-4">Maak een uitnodigingslink en deel die met je collega. De link is 14 dagen geldig en koppelt het account automatisch aan {org.name}.</p>
              <InviteForm orgId={org.id} />
              {(invites ?? []).length > 0 && (
                <div className="mt-6">
                  <p className="t-label mb-2">Openstaande uitnodigingen</p>
                  <ul className="divide-y divide-line">
                    {(invites ?? []).map((i) => (
                      <li key={i.id} className="py-2 flex flex-wrap items-center gap-3 text-sm">
                        <span className="font-semibold">{i.email}</span>
                        <span className="text-xs t-muted">{ROLE_LABELS[i.role as OrgRole]} · verloopt {fmtDate(i.expires_at)}</span>
                        <code className="text-xs bg-cloud px-2 py-1 rounded-lg select-all break-all">{base}/invite/{i.token}</code>
                        <form action={revokeInvite} className="ml-auto"><input type="hidden" name="id" value={i.id} /><button type="submit" className="text-xs t-muted hover:text-coral-deep font-semibold">Intrekken</button></form>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <Panel eyebrow="Teams" title="Teams beheren"><p className="text-sm t-muted">Teams, leden en leads beheer je op de <Link href="/teams" className="text-blue-deep font-semibold hover:underline">teampagina&apos;s</Link>.</p></Panel>
          </>
        )}
      </div>
    </div>
