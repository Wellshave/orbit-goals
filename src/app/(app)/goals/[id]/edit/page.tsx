...
export default async function EditGoalPage({ params }: PageProps<"/goals/[id]/edit">) {
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader eyebrow="Doel bewerken" title={g.title} />
      ...
      <form action={deleteGoal} className="mt-6 deck p-4 flex items-center justify-between gap-4">
        <input type="hidden" name="id" value={g.id} />
        <p className="text-sm text-muted">Verwijderen wist ook alle updates, milestones en reacties van dit doel.</p>
        <button type="submit" className="text-sm font-semibold text-coral-soft hover:underline shrink-0">Doel verwijderen</button>
      </form>
    </div>
  );
}
