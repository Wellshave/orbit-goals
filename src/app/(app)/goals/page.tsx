...
export default async function GoalsPage({ searchParams }: PageProps<"/goals">) {
  ...
  const typeHref = (ty: string) => `/goals?${base}${ty ? `&type=${ty}` : ""}${fStatus ? `&status=${fStatus}` : ""}`;

  return (
    <div className="pt-2">
      <PageHeader help="goals" icon="rocket" tone="coral" eyebrow={t("goals.title")} title={t("goals.heading")} description={t("goals.sub")} actions={<ButtonLink href="/goals/new" size="sm"><Plus className="size-4" aria-hidden /> {t("goals.newGoal")}</ButtonLink>}>
        <div className="flex flex-col gap-4">
          <PeriodBar current={period.key} label={period.label} />
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex flex-wrap items-center gap-2" data-tour="type-filter">{[["", t("common.all")], ["personal", t("goalType.personal")], ["team", t("goalType.team")], ["company", t("goalType.company")]].map(([v, l]) => (
            ...
            <span className="inline-flex flex-wrap items-center gap-2" data-tour="status-filter">{STATUS_ORDER.map((s) => (
              <Link key={s} href={`/goals?${base}${fType ? `&type=${fType}` : ""}${fStatus === s ? "" : `&status=${s}`}`} className={`press rounded-full px-3 py-1.5 text-xs font-semibold ${fStatus === s ? "ring-2 ring-ink/40" : ""}`}>
                <Chip tone={STATUS_META[s].tone}>{t(`status.${s}`)} · {counts[s]}</Chip>
              </Link>
            ))}</span>
          </div>
          <details className="group">
            <summary className="cursor-pointer text-sm font-semibold text-blue-deep list-none [&::-webkit-details-marker]:hidden">{t("goals.moreFilters")}</summary>
            <form method="get" className="mt-3 flex flex-wrap items-end gap-2">
              ...
              <label className="sr-only" htmlFor="f-q">{t("common.search")}</label>
              <input id="f-q" name="q" defaultValue={q} placeholder={t("common.search")} className="ctl !w-44 !py-2 text-sm" />
              <label className="sr-only" htmlFor="f-person">{t("common.person")}</label>
              <select id="f-person" name="person" defaultValue={fPerson} className="ctl !w-auto !py-2 text-sm"><option value="">{t("common.everyone")}</option>{dir.members.map((m) => <option key={m.id} value={m.id}>{m.full_name}</option>)}</select>
              <label className="sr-only" htmlFor="f-team">{t("common.team")}</label>
              <select id="f-team" name="team" defaultValue={fTeam} className="ctl !w-auto !py-2 text-sm"><option value="">{t("common.allTeams")}</option>{dir.teams.map((tm) => <option key={tm.id} value={tm.id}>{tm.name}</option>)}</select>
              <label className="sr-only" htmlFor="f-cat">{t("common.category")}</label>
              <select id="f-cat" name="category" defaultValue={fCat} className="ctl !w-auto !py-2 text-sm"><option value="">{t("common.allCategories")}</option>{categories.map((c) => <option key={c} value={c}>{c}</option>)}</select>
              ...
              {(fPerson || fCat || fTeam || q || fType || fStatus) && <Link href={`/goals?period=${period.key}`} className="text-sm t-muted hover:text-ink">{t("common.clear")}</Link>}
            </form>
          </details>
          <SavedViews filters={(filtersRes.data ?? []) as SavedFilter[]} />
        </div>
      </PageHeader>
      <p className="t-muted text-sm mb-4">{t("goals.count", { a: filtered.length, b: goals.length })}</p>
      {filtered.length === 0 ? (
        <EmptyState icon="rocket" tone="coral" title={t("goals.none")} body={goals.length === 0 ? t("goals.noneFirst") : t("goals.noneFilter")} action={<ButtonLink href="/goals/new" size="sm">{t("goals.newGoal")}</ButtonLink>} />
      ) : (
